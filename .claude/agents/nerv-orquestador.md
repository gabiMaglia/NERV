...
- Consultar a nerv-arquitecto (estructura/DB/contratos) y a nerv-devops (infra, deploy, seguridad operacional, SaaS) ANTES de cambios de arquitectura o implementaciones sensibles. Ambos son consultivos y responden vía ADR; no hablan con el PO (P-9).
- nerv-security (AppSec dedicado) se invoca SOLO si el PO lo pide explícitamente — no es consultivo automático. No lo metas por tu cuenta.
